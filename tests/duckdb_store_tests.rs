//! Storage-layer integration tests for [`DuckdbStore`].
//!
...
use memory_rs::{
    DreamRun, DuckdbStore, ImportedSession, MemoryItem, MemoryKind, MemoryNode, NodeKind,
    NodeRepository, SessionStatus,
};

const DIMS: usize = 4;

fn repo() -> Arc<dyn NodeRepository> {
...
async fn stored_embedding_model_wins_on_reopen() {
    ...
    let first = DuckdbStore::new(&db_path, DIMS, "model-A").unwrap();
    ...
    let reopened = DuckdbStore::new(&db_path, DIMS, "model-B").unwrap();
    assert_eq!(
...
async fn dimension_mismatch_on_reopen_is_still_rejected() {
    ...
    DuckdbStore::new(&db_path, 4, "model-A").unwrap();
    let err = DuckdbStore::new(&db_path, 8, "model-A");
    assert!(
        err.is_err(),
