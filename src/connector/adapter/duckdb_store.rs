//! DuckDB-backed [`NodeRepository`](crate::application::NodeRepository).
//!
...
//! it (see [`NodeRepository::embedding_model`]) rather than being rejected.
...
use crate::application::{NodeRepository, NodeStats};
use crate::domain::{
...
pub struct DuckdbStore {
    // `pub(crate)` so the memory adapter, which implements a second port on this
    // same struct, shares the one connection rather than opening its own.
...
impl DuckdbStore {
    ...
    fn initialize(
        ...
        // [`NodeRepository::embedding_model`] and embed queries with it.
    ...
    /// Synchronous companion to [`NodeRepository::embedding_model`], so the
    ...
    pub(crate) fn vector_literal(&self, vector: &[f32]) -> Result<String, DomainError> {
    ...
    pub(crate) async fn query_blocking<T, F>(&self, f: F) -> Result<T, DomainError>
    where
...
pub(crate) fn project_from_column(stored: String) -> Option<String> {
...
#[async_trait]
impl NodeRepository for DuckdbStore {
...
pub(crate) fn project_scope_clause(column: &str, projects: Option<&[String]>) -> Option<String> {
